...
def testa_conexao_mongodb(db, dbcollection):
...
def inserir_df_no_mongodb_sem_duplicados_save_csv(df, collection, campos_unicos):
    """
    ...
    csv_file_path = r'app\data\dataset\data_webscraping.csv'
    
    # Verificar se o arquivo CSV já existe e se possui conteúdo
    ...
    for documento in documentos:
        ...
        if collection.count_documents(filtro) == 0:
            ...
            # Converter o documento (dicionário) de volta em um DataFrame com uma única linha
            df_temp = pd.DataFrame([documento])
            
            # Verificar se o arquivo já tem cabeçalho (com base na existência e no tamanho)
            if file_exists:
                # Se o arquivo já existe e tem dados, escrever sem o cabeçalho
                df_temp.to_csv(csv_file_path, mode='a', header=False, index=False)
            else:
                # Caso contrário, escrever com o cabeçalho
                df_temp.to_csv(csv_file_path, mode='a', header=True, index=False)
                file_exists = True  # Após a primeira inserção, o arquivo já existe e tem dados
        else:
...
if __name__ == '__main__':
    ...
    mongodb_collection = testa_conexao_mongodb('CMP', 'EMPENHOS_DETALHADOS_TESTE')
    ...
    inserir_df_no_mongodb_sem_duplicados_save_csv(df_completo, mongodb_collection, chave_composta)
